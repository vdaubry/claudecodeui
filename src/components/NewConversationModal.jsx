 ...
 *
 * Collects the first message from the user before creating a conversation.
 * Uses pure REST call to create conversation with session ID synchronously.
 * Reuses MessageInput component for full feature parity.
 */
...
export default function NewConversationModal({
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      console.error('[NewConversationModal] Error:', err);
      setError(err.message);
      setIsSending(false);
    }
  }, [input, isSending, taskId, projectPath, permissionMode, onConversationCreated]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-2xl mx-4 p-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          Start a new conversation in <span className="font-medium text-gray-700 dark:text-gray-300">{project?.name || 'this project'}</span>
        </p>
